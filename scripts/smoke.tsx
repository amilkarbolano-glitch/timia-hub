...
(async () => {
  ...
  for (const acc of auth.MOCK_ACCOUNTS) {
    ...
    for (const [name, C, props] of [['Plan', PlanDeTrabajo, {}], ['Estim', Estimaciones, { onViewChange:()=>{}, onBack:()=>{} }]] as any) {
      ...
      try {
        ...
        for (const p of ['FICO','CRONOS','NGA','FICO','Consolidado','Input','FICO 2.0','Randómico','Principal','+ Bloqueante','+ Alerta','+ Cronograma']) await click(p);
